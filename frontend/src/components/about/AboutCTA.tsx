...
import { PublicAboutCTA } from "@/services/about.service";
...
export const AboutCTA: React.FC<AboutCTAProps> = ({ data }) => {
  if (data?.visible === false) return null;

  const ctaBg = defaultCtaBackground;
  const eyebrow = data?.eyebrow ?? "JOIN THE WATER REVOLUTION";
  const title =
    data?.title ?? "The future of water is intelligent, accountable, and sustainable.";
  const description =
    data?.description ||
    "Join forward-thinking enterprises and utilities creating water visibility and verifiable sustainability with Veenero.";
  const primaryButtonText = data?.primaryButtonText || "Join Us in Our Mission";
    ...
    >
      <div className="container mx-auto px-6 md:px-12 max-w-7xl font-sans">
        <div className="relative overflow-hidden bg-slate-950 rounded-3xl p-8 sm:p-12 md:p-14 shadow-2xl border border-teal-900/40">
          ...
          {/* Background Planet & Water Splash Image (solutions-cta-background.png) */}
