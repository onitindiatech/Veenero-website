...
import {
  syncAllCmsReferences,
  scanRealCmsUsage,
  isAssetReferencedInCms,
} from '../services/mediaSync.service';
...
export const replaceMediaHandler = async (
  ...
  try {
    ...
    const previousSecureUrl    = mediaDoc.secureUrl;
    ...
    // Synchronize all CMS references atomically across all collections
    try {
      await syncAllCmsReferences(previousPublicId, previousSecureUrl, {
        publicId: cloudinaryResult.publicId,
        secureUrl: cloudinaryResult.secureUrl,
        page: mediaDoc.page,
        section: mediaDoc.section,
...
export const deleteMediaHandler = async (
  ...
  try {
    ...
    // Safety check: verify if asset is actively referenced in real CMS pages
    const { isReferenced, usages } = await isAssetReferencedInCms(
      mediaDoc.publicId,
      mediaDoc.secureUrl,
      mediaDoc.page,
      mediaDoc.section,
    ...
    if (isReferenced && req.query.force !== 'true') {
      const usageSummary = usages.map((u) => `${u.page.toUpperCase()} (${u.section} > ${u.field})`).join(', ');
      throw new ApiError(
        400,
        `Cannot delete media asset: It is actively referenced in CMS on: ${usageSummary}. Reassign or remove the image in CMS before deleting, or use force confirmation.`
      );
    }
...
export const hardDeleteMediaHandler = async (
  ...
  try {
    ...
    if (isReferenced && req.query.force !== 'true') {
      ...
      throw new ApiError(
        400,
        `Cannot permanently delete media asset: It is actively referenced in CMS on: ${usageSummary}. Reassign or remove the image in CMS before deleting, or use force confirmation.`
      );
    }
...
export const getMediaUsageHandler = async (
  ...
  try {
    ...
    const usages = await scanRealCmsUsage(
      publicId,
      mediaDoc.secureUrl,
      mediaDoc.page,
      mediaDoc.section,
